...
class TFHubClient:
    ...
    def get_image_classification_model_from_cache_else_load(self, model_name: str):
        with tf.device("/gpu:0"):
           is_model_cached, model_path = self._is_model_cached("image_classification", model_name)
           if is_model_cached:
    ...
    def get_object_detection_model_from_cache_else_load(self, model_name: str):
        with tf.device("/gpu:0"):
           is_model_cached, model_path = self._is_model_cached("object_detection", model_name)
           if is_model_cached:
    ...
    def get_imagenet_classes(self) -> List[str]:
        with tf.device("/gpu:0"):
           ...
           labels_file = "api/image_features/ImageNetLabels.txt"
